...
See [CUTOFF_TIME_VALIDATION.md](/docs/CUTOFF_TIME_VALIDATION.md) for detailed explanation.
...
**Problem**: Referenced shipping method doesn't exist.

**Solution**: Create the shipping method first and use `depends_on`:
...
resource "emporix_delivery_time" "friday" {
  ...
  depends_on = [emporix_shipping_method.standard]
}
```

### Error: Shipping zone not found

**Problem**: Referenced zone doesn't exist.
...
**Problem**: Time not in HH:MM:SS format.

**Solution**: Always use 24-hour format with seconds:
```terraform
delivery_time_range = {
  time_from = "10:00:00"  # ✅ Correct
  time_to   = "14:00:00"  # ✅ Correct
}
```

### Error: Cut-off time validation failed

**Problem**: Cut-off time is after delivery starts.

**Solution**: Set cut-off time before delivery:
```terraform
# Delivery: 10:00-12:00
cut_off_time = {
  time = "2023-06-12T06:00:00.000Z"  # ✅ 6 AM (before 10 AM)
  # NOT "2023-06-12T18:00:00.000Z"    # ❌ 6 PM (after delivery)
}
```

## Summary

✅ **Prerequisites** - Shipping zone and shipping method must exist first
✅ **Dependencies** - Use `depends_on` to ensure correct creation order
✅ **Time format** - HH:MM:SS required for delivery times
✅ **Cut-off logic** - Must be before delivery window starts
✅ **Weekdays** - Use uppercase day names (MONDAY, TUESDAY, etc.)
✅ **Zone or all zones** - Specify either zone_id or is_for_all_zones

Delivery times enable sophisticated scheduling of deliveries with capacity management and multiple shipping methods per time slot.
